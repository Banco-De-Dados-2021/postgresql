...
ALTER TABLE "Setor" ALTER COLUMN id SET DEFAULT nextval('seq_setor');

CREATE SEQUENCE SEQ_PRODUTO;
ALTER TABLE "Produto" ALTER COLUMN id SET DEFAULT nextval('seq_produto');

CREATE SEQUENCE SEQ_ESTOQUE;
ALTER TABLE "Estoque" ALTER COLUMN id SET DEFAULT nextval('seq_estoque');

CREATE SEQUENCE SEQ_UNIDADE_MEDIDA;
ALTER TABLE "UnidadeMedida" ALTER COLUMN id SET DEFAULT nextval('seq_unidade_medida');

CREATE SEQUENCE SEQ_FORNECEDOR;
ALTER TABLE "Fornecedor" ALTER COLUMN id SET DEFAULT nextval('seq_fornecedor');
...
insert into "Setor" (id, nome)
			...
			(4, 'Frios'),
			(5, 'Açougue'),
			(6, 'Aves, frios e laticinios'),
			(7, 'Confeitaria'),
			(8, 'Pescados');


insert into "Produto" (id, nome, vencimento, setor_fk, unidade_medida_fk) values
                (1, 'Arroz', current_date + interval '1 year', 2, 1),
                (2, 'Azeite',               current_date + interval '1 year', 2, 6   ),
                (3, 'Bolachas e biscoitos', current_date + interval '1 year', 2, 1   ),
                (4, 'Café',                 current_date + interval '1 year', 2, 1   ),
                (5, 'Chá',                  current_date + interval '1 year', 2, 1   ),
                (6, 'Extrato de tomate',    current_date + interval '1 year', 2, 1   ),
                (7, 'Farinha',              current_date + interval '1 year', 2, 2   ),
                (8, 'Feijão',               current_date + interval '1 year', 2, 1   ),
                (9, 'Leite',                current_date + interval '1 year', 2, 8   ),
                (10, 'Macarrão',             current_date + interval '1 year', 2, 1   ),
                (11, 'Maionese',             current_date + interval '1 year', 2, 1   ),
                (12, 'Óleo',                 current_date + interval '1 year', 2, 6  ),
                (13, 'Tempero pronto',       current_date + interval '1 year', 2, 1   ),
                (14, 'Pao de Queijo Congelado',       current_date + interval '1 year', 4, 9   ),
                (15, 'Batata Palito',       current_date + interval '1 year', 4, 9   ),
                (16, 'Hamburguer',       current_date + interval '1 year', 4, 1   ),
                (17, 'Lasanha',       current_date + interval '1 year', 4, 1   ),
                (18, 'Pizza',       current_date + interval '1 year', 4, 1   );

insert into "Estoque" (id, produto_fk, quantidade) values
                        (1, 1, 100),
                        (2, 2, 20),
                        (3, 3, 500),
                        (4, 4, 8),
